...
func setup() (*sql.DB, *sql.DB, error) {
	...
	// Connect to the databases
	userdb, err := sql.Open("pgx", "postgres://user1:secret@localhost:5434/user1?sslmode=disable")
	if err != nil {
		return nil, nil, err
	}
	...
	orderdb, err := sql.Open("pgx", "postgres://user2:secret@localhost:5434/user2?sslmode=disable")
	if err != nil {
		return nil, nil, err
	}
